...
import type { WorkshopInspection } from '../../../types/database';
...
export function HSWorkshop({ onBack }: Props) {
  const [sites, setSites] = useState<Site[]>([]);
  const [rows, setRows] = useState<WorkshopInspection[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [showForm, setShowForm] = useState<null | { site: Site }>(null);
  const [editing, setEditing] = useState<WorkshopInspection | null>(null);
  ...
  const fetchAll = async () => {
    ...
    setError(null);
    try {
      const [{ data: s, error: sErr }, { data: insp, error: iErr }] = await Promise.all([
        supabase.from('sites').select('id,name').order('name'),
        supabase.from('workshop_inspections').select('*').order('check_date', { ascending: false }),
      ]);
      if (sErr || iErr) throw sErr || iErr;
      setSites(s || []);
      setRows(insp || []);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load workshop inspections');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleViewPDF = async (row: WorkshopInspection) => {
    try {
      setPdfError(null);
      ...
      doc.setFontSize(16);
      doc.text('Workshop Inspection', 14, 18);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Workshop Inspections</h2>
      </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Workshop</th>
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {sRows.map(r => (
                          <tr key={r.id}>
                            ...
                            <td className="px-4 py-3 text-sm">{r.workshop_name || '-'}</td>
...
type FormProps = { site: Site; editing: WorkshopInspection | null; onClose: () => void; onSaved: () => void };

function WorkshopInspectionForm({ site, editing, onClose, onSaved }: FormProps) {
  ...
  const defaultNames: string[] = [
    ...
    'Electrical Switches, Plugs, Sockets & RCDs',
    'Hazardous Substances information and PPE',
    'Fixed Machines',
    'Storage Racking / A Frames / T-racks',
    'PPE Condition and Use',
    'Floors - clean, tidy and pothole free',
    'Fire Exits - signs & exits serviceable',
    'Fire Fighting Equipment',
    'Emergency Procedures',
    'Machinery Guarding',
    'First Aid Facilities',
    'Sufficient First Aiders?',
    'Sanitary & Welfare Facilities',
    'Local Exhaust Ventilation Units',
    'Ladders / Steps',
    'Crane / Lifting Gear',
  ];
  ...
  useEffect(() => {
    ...
    setWorkshopName(editing.workshop_name || '');
  ...
  const validateStep = (step: number) => {
    setError(null);
    if (step === 1) {
      if (!createdByName || !checkDate || !workshopName) {
        setError('Please provide inspector, date and workshop name.');
        return false;
      }
    }
    if (step === 2) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editing) {
        const { error } = await supabase.from('workshop_inspections').update(payload).eq('id', editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('workshop_inspections').insert([payload]);
        if (error) throw error;
      }
      onSaved();
    } catch (e) {
  ...
  return (
    <FormContainer isOpen={true} maxWidth="3xl">
      <FormHeader title={`Workshop Inspection — ${site.name}`} onClose={onClose} />
      <FormContent>
        ...
        {currentStep === 1 && (
          <div className="space-y-4">
            ...
            <TextInput label="Workshop Name" value={workshopName} onChange={setWorkshopName} required />
            <RadioGroup
